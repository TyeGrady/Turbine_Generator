function [Section,Re] = XYZ_Generator_V6_Matrix()
...
totalLength=1.75; % inches total length
chord_max=0.75; % inches - the maximum chord width
X = 9; %Tip Speed Ratio (R*omega/V_0) (between 5 and 10)
V_0 = 40; %Free Stream Velocity, mph
...
V_e = 10; %Wind speed at end of the blade [mph]
...
%row 3 - x_move
%row 4 - rotation
%row 5 - apparent wind speed seen by section

Section = zeros(5,detail);
Re = zeros(1,detail);
V_0 = 17.6 * V_0; %Convert mph to in/s
...
for l=0:(totalLength/(detail-1)):(totalLength)
    ...
    C_opt = ((2*pi()*(Section(1,i)+hub_r)*8*V_0)/(n*9*Cl*X*W));
...
Root_T_S = 0.60; %Percent of max. chord width at beginning of root
Root_T_E = 1; %Percent of max. chord width at end of root
...
Shrink_T_E = 0.01; %Percent of max. chord width at end of root
...
function [theta] = Twist_Exp(l,hub_r,totalLength,X,V_0)
...
Alpha = -20 * ((2*pi())/360); %Angle of attack of 20 deg, converted to rads
...
w = r*Omega;
phi = (2/3)*atan(R/(X*r));
phi_0 = (2/3)*atan(R/(X*hub_r));
theta = -phi -alpha + phi_0 - (90*(pi/180));
