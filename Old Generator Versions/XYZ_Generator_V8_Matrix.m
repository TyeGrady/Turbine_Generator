function [Section,Re] = XYZ_Generator_V8_Matrix()
...
mkdir('Profile_Files_' & dt)
cd('.\Profile_Files_' & dt);
...
for l=0:(totalLength/(detail-1)):(totalLength)
    ...
    iaf.wantFile=1;
    iaf.datFilePath='.\Profile_Files_' & dt; % Current folder
    iaf.is_finiteTE=0;
    blade = naca4gen(iaf);
